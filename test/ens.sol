...
contract DeployENS {
    ...
    function DeployENS() {
        ...
        resolver.setName(ournode, "deployer.puffs");
        ...
        // Set foo.puffs up with a resolver, ABI, and addr record
        ens.setSubnodeOwner(0, tld, this);
        ens.setSubnodeOwner(tldnode, sha3('foo'), this);
        var fooDotPuffs = sha3(tldnode, sha3('foo'));
        ens.setResolver(fooDotPuffs, resolver);
        resolver.setAddr(fooDotPuffs, this);
        resolver.setABI(fooDotPuffs, 1, '[{"constant":true,"inputs":[],"name":"test2","outputs":[{"name":"","type":"uint256"}],"payable":false,"type":"function"},{"inputs":[],"payable":false,"type":"constructor"}]');
